...
class Fragment:
    ...
    def __post_init__(self):
        self.atom_index = [int(x) for x in self.atom_index]
...
def find_brics_fragments(mol):
    ...
    brics_bonds = list(
        BRICS.FindBRICSBonds(mol)
    )  # convert generator to list for later purposes
    ...
    if len(atom_tuples) == 0:
        fragments = [
            Fragment(
                atom_index=list(range(mol.GetNumAtoms())), mol=mol, environment="na"
            )
        ]
    ...
    fragments = [
        Fragment(atom_index=n, mol=m) for (n, m) in zip(fragment_atoms, fragment_mols)
    ]
        ...
        for atom, atom_number in zip(atoms, atom_numbers):
            ...
            if atom.GetSymbol() != "*":  # regular atom
                ...
                atom_number, subpocket, environment = _get_atom_properties_regular(
                    atom_number, fragment, brics_fragments
                )
                ...
                atom_number, subpocket, environment = _get_atom_properties_dummy(
                    atom_number, atom, atom_tuples, fragments
                )
                atom.SetIntProp("atomNumber", atom_number)
                atom.SetProp("subpocket", subpocket)
                atom.SetProp("environment", environment)
...
def _get_atom_properties_regular(atom_number, fragment, brics_fragments):
    ...
    environments = next(
        ...
        for brics_fragment in brics_fragments
    ...
    environment = next(
        (env for atom_id, env in environments if atom_id == atom_number),
        next(iter(environments))[1],
    )
...
def _get_atom_properties_dummy(atom_number, atom, atom_tuples, fragments):
    ...
    bonds_involving_dummy_connected_atom = _get_bonds_from_dummy_connected_atom(
        dummy_connected_atom_number, atom_tuples
    )
    ...
    dummy_replaced_atom_numbers = _get_dummy_replaced_atoms_from_bonds(
        dummy_connected_atom_number, bonds_involving_dummy_connected_atom
    )

    # if multiple dummy atoms at dummy-connecting atom, select the correct dummy-replaced atom number (int)
    dummy_replaced_atom_number = _select_dummy_replaced_atom(
        dummy_replaced_atom_numbers, atom_number, fragments
    )

    # get subpocket of the dummy-replaced atom
    subpocket = _get_dummy_replaced_atom_subpocket(
        dummy_replaced_atom_number, fragments
    )
...
def _get_dummy_connected_atom_from_dummy_atom(atom):
    ...
    if len(dummy_connected_atoms) == 1:
        ...
        raise ValueError(
            f"Unexpected number of dummy-connected atoms: {len(dummy_connected_atoms)}"
        )
...
def _get_dummy_replaced_atoms_from_bonds(
    dummy_connected_atom_number, atom_tuples_involving_neighbor
):
    """
    ...
    for atom_tuple_involving_neighbor in atom_tuples_involving_neighbor:
        ...
        dummy_replaced_atom_number = [
            atom_number
            for atom_number in atom_tuple_involving_neighbor
            if atom_number != dummy_connected_atom_number
        ]

        if len(dummy_replaced_atom_number) != 1:
            raise ValueError(
                f"Unexpected number of dummy-replaced atoms: {len(dummy_replaced_atom_number)}"
            )

        dummy_replaced_atom_numbers.extend(dummy_replaced_atom_number)

    return dummy_replaced_atom_numbers


def _select_dummy_replaced_atom(
    dummy_replaced_atom_numbers, dummy_atom_number, fragments
):
    """
    ...
    # in very rare cases more than one dummy atom per dummy-connected atom
        ...
        selected_dummy_replaced_atom_number = (
            []
        )  # Should have 1 element at the end of iteration

        dummy_atom_position = _get_atom_position_from_atom_number(
            dummy_atom_number, fragments
        )

        for dummy_replaced_atom_number in dummy_replaced_atom_numbers:
            dummy_replaced_atom_position = _get_atom_position_from_atom_number(
                dummy_replaced_atom_number, fragments
            )

            if np.isclose(
                dummy_replaced_atom_position.Length(),
                dummy_atom_position.Length(),
                rtol=1e-04,
            ):
        ...
        if len(selected_dummy_replaced_atom_number) != 1:
            raise ValueError(
                f"Unexpected number of selected dummy-replaced atoms: "
...
def _get_dummy_replaced_atom_subpocket(dummy_replaced_atom_number, fragments):
    ...
    if len(dummy_replaced_atom_subpockets) == 1:
        ...
        raise ValueError(
            f"Unexpected number of subpockets: {len(dummy_replaced_atom_subpockets)}"
        )
