...
def run_experiment(
    ...
    for seed in seeds:
        # 1. Force explicit login on the cluster node
        api_key = os.environ.get("WANDB_API_KEY")
        if api_key:
            wandb.login(key=api_key)

        # 2. Initialize wandb (removed the deprecated reinit=True)
        wandb.init(
            project="bcos-gnn-ba2motif",
            name=f"{experiment_name}_seed_{seed}",
        ...
        # Fine-grained logging inside the training loop
        for epoch in range(1, epochs + 1):
            ...
            wandb.log({
                ...
                "val/clf_auroc": val_metrics["clf_auroc"],
                "lr": current_lr
            })
        ...
        if do_expl_metrics:
            ...
            test_jaccard = float(
                evaluate_jaccard(eval_model_cpu, test_data, method="explain", transform=transform),
            )
            test_expl_auroc = float(
                evaluate_auroc(eval_model_cpu, test_data, method="explain", transform=transform),
            )
            model.to(DEVICE)
            eval_model = EvalModelAdapter(model).to(DEVICE)
        else:
        ...
        # Log final test metrics for this run to wandb
        wandb.log({
            "test/loss": result["test_loss"],
            "test/acc": result["test_acc"],
            "test/clf_auroc": result["test_clf_auroc"],
            "test/expl_auroc": result["test_expl_auroc"],
            "test/jaccard": result["test_jaccard"],
            "best_epoch": best_epoch,
        })
...
def main():
    ...
    seed_results_csv = RESULTS_DIR / "ba2motif_all_seed_results.csv"
    summary_csv = RESULTS_DIR / "ba2motif_summary_mean_std.csv"
    combined_csv = RESULTS_DIR / "ba2motif_all_results_with_summary.csv"
    ...
    for experiment_name, best_result in best_by_experiment.items():
        ...
        completeness = run_bcos_node_completeness_check(
            ...
            transform=transform,
            device="cpu",
            title=f"B-COS Completeness Check ({experiment_name}, seed={best_result['seed']})",
