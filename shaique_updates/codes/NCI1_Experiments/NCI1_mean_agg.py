...
class EvalModelAdapter(torch.nn.Module):
	...
	def forward(self, x, edge_index, edge_attr=None, batch=None):
		...
		return self.base_model(x.float(), edge_index, batch)
...
def train_one_epoch(model, loader, criterion, optimizer):
	...
	for batch in loader:
		...
		total_correct += int((pred == batch.y.view(-1).float()).sum().item())
...
def evaluate_loader(model, loader, criterion, num_classes: int):
	...
	try:
		clf_auroc = float(
			roc_auc_score(labels_all, probs),
		)
	except ValueError:
		clf_auroc = float("nan")
...
def run_experiment(
	...
	for seed in seeds:
		...
		scheduler = ReduceLROnPlateau(
			optimizer,
			mode="max", # We will monitor acc since we are doing multiclass/CrossEntropy usually
			factor=0.5,
			patience=10,
			min_lr=1e-6,
		)
		...
		for epoch in range(1, epochs + 1):
			...
			val_metrics = evaluate_loader(model, val_loader, criterion, num_classes)

			current_lr = optimizer.param_groups[0]["lr"]
			# Use accuracy for scheduler and early stopping just like notebook
		...
		test_metrics = evaluate_loader(model, test_loader, criterion, num_classes)
...
def main():
	...
	seeds = [42, 123, 999] # From bcos_NCI1.ipynb
	epochs = 200
	batch_size = 128 # From bcos_NCI1.ipynb
	lr = 1e-3
	hidden_dim = 64 # From bcos_NCI1.ipynb
	b = 2.0
	early_stop_patience = 25
	min_delta = 0.0 # From early stopper in notebook
	...
	seed_results_csv = RESULTS_DIR / "nci1_all_seed_results.csv"
	summary_csv = RESULTS_DIR / "nci1_summary_mean_std.csv"
	combined_csv = RESULTS_DIR / "nci1_all_results_with_summary.csv"
