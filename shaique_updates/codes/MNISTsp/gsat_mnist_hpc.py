...
def load_saved_splits(repo_root: Path | None, script_dir: Path, batch_size: int, add_pos_features: bool = True):
    candidate_roots = []

    env_split_root = os.environ.get("MNISTSP_SPLIT_ROOT", "").strip()
    if env_split_root:
        candidate_roots.append(Path(env_split_root).expanduser())
    ...
    candidate_roots.extend([
        script_dir / "data" / "MNIST" / "sparsified_pt_splits",
        Path.cwd() / "data" / "MNIST" / "sparsified_pt_splits",
    ])
...
def run(args):
    ...
    repo_root = find_repo_root(cwd) or find_repo_root(script_dir)

    if repo_root is not None:
        print(f"Repo root found: {repo_root}")
    else:
        print("Repo root not found; using local/script data paths for saved splits.")
    ...
    print("\n" + "=" * 78)
    print("FULL TEST-SET TIMING SUMMARY ACROSS SEEDS")
    print("=" * 78)
